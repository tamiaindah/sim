...
<body>
    <div class="container">
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" name="user_berita" value="<?php echo $_SESSION['username'] ?>" readonly>
            </div>

        <div class="form-group">
            <label for="exampleInputEmail1">Tanggal Berita</label>
            <input type="text" class="form-control" name="tanggal_berita" value="<?php echo date('d-m-Y') ?>" readonly>
            </div>

        <div class="form-group">
                <label for="exampleInputPassword1">Isi Berita</label>
                <textarea name="isi_berita" class="ckeditor"></textarea>
            </div>
    ...
    <?php
       ...
       if(isset($_POST['submit'])) {
        ...
        if(!in_array($extension, $file_extension)) {
            ...
            if (!$size < 1050050) {
                $foto_berita = $rand.'_'.$filename;
                move_uploaded_file($_FILES['foto_berita']['tmp_name'], "../../img_berita/".$foto_berita);
                mysqli_query($koneksi, "INSERT into tb_berita values('$id_berita', '$judul_berita', '$user_berita', 
                                                                    '$tanggal_berita', '$isi_berita', '$foto_berita')");
