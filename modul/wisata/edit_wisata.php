...
<body>
    <div class="container">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            ...
            <input type="text" class="form-control" name="id_wisata" value="<?php echo $a['id_wisata'] ?>">
            </div>
          <div class="form-group">
            <label for="exampleInputEmail1">Nama Wisata</label>
            <input type="text" class="form-control" name="nama_wisata" value="<?php echo $a['nama_wisata'] ?>" >
            </div>
        ...
        <div class='gllpLatlonPicker'>
            ...
            <input type='text' name='longtitute' class='gllpLongitude' value='110.36403' readonly/> &#8592 <font color='red'>Longitude</font> <br/> <br/>
            ...
            if ($a['kategori_wisata']==0) { ?>
                  <option value="0">---Pilih Kategori---</option>
                  <?php 
                  }  
            while ($r=mysqli_fetch_array($tampil)) {

                if ($a["kategori_wisata"]==$r['id_kategori']) { ?>
             <option value="<?php echo $r['id_kategori'] ?>" selected><?php echo $r['nama_kategori'] ?></option>
            <?php
            }else { ?>
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="lokasi_wisata" value="<?php echo $a['lokasi_wisata'] ?>">
                </div>

            <div class="form-group">
                    <label for="exampleInputPassword1">DESKRIPSI</label>
                    <textarea name="deskripsi_wisata" class="ckeditor" value="<?php echo $a['deskripsi_wisata'] ?>"></textarea>
                </div>
        ...
        <div class="form-group">
            ...
            <img src="../../img_wisata/<?php echo $a['foto_wisata'] ?>" height="100" width="100"></img>
...
if ($filename != "" ){
            ...
            if (!$size < 1050050) {
                    ...
                    $query = mysqli_query($koneksi, "SELECT * FROM tb_wisata WHERE id_wisata='$id_wisata'");
                    $a = mysqli_fetch_array($query);

                    $img_name = $a['foto_wisata'];
                    $loc = "../../img_wisata/$img_wisata";

                    @unlink($loc);

                    $nama_gambar = $rand.'_'.$filename;
                    move_uploaded_file($_FILES['foto_wisata']['tmp_name'], "../../img_wisata/".$nama_gambar);

                    mysqli_query($koneksi, "UPDATE tb_wisata SET nama_wisata='$nama_wisata',
                                                                kategori_wisata='$kategori_wisata',
                                                                lokasi_wisata='$lokasi_wisata',
                                                                latitude='$latitude',
                                                                longitude='$longitude',
                                                                foto_wisata='$nama_gambar',
